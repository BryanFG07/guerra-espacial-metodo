...
function oprimir_btn(i) {
  if (posibles_respuestas[i] == pregunta.respuesta) {
    ...
    Swal.fire({
      ...
      text:'La nave enemiga lo ha capturado y no pudo ganar su libertad, tu puntuación es: 221',
...
function TiempoPreguntas(){
  ...
  if(tiempop==0){
    document.getElementById('tiempop').style.display="none";
        Swal.fire({
            title: 'Perdiste',
            text:'El tiempo se ha agotado, tu puntuación es: 221',
            icon:'error',
...
ene2.style.transformStyle='0.3s';
...
setInterval(Mover,900);
setInterval(Mover2,900);
...
function Pregunta(){
  ...
  document.getElementById('tiempo').style.display="none";
    Swal.fire({
        ...
        if (result.value) {
            window.location = "/preguntas9.html";
        }
    });
}
function siguiente(){

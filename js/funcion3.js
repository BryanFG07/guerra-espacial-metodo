...
function oprimir_btn(i) {
  if (posibles_respuestas[i] == pregunta.respuesta) {
    ...
    Swal.fire({
      ...
      text:'La nave enemiga lo ha capturado y no pudo ganar su libertad, tu puntuación es: 35',
...
function TiempoPreguntas(){
  ...
  if(tiempop==0){
    document.getElementById('tiempop').style.display="none";
        Swal.fire({
            title: 'Perdiste',
            text:'El tiempo se ha agotado, tu puntuación es: 35',
            icon:'error',
...
function Pregunta(){
    ...
    Swal.fire({
        ...
        if (result.value) {
            window.location = "/preguntas3.html";
        }
    });
}
function siguiente(){
    ...
    Swal.fire({
        title: 'Felicidades',
        text:'Ha completado el nivel '+(Nivel),
        icon:'success',
